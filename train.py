...
from test_time_training import ttt_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    for ticker in ret_window.columns:
        s = ttt_score(
            ...
            ttt_lr=config.TTT_LEARNING_RATE,
    ...
    top_etfs = [{"ticker": t, "ttt_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/ttt_{timestamp}.json"
