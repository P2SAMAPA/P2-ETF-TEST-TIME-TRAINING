...
st.set_page_config(page_title="Test-Time Training Layers", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🧠 Test-Time Training Layers</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Self-supervised adaptation during inference | Real‑time distribution shift</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 TTT")
...
st.sidebar.markdown(f"**Seq len:** {config.SEQ_LEN} | **TTT steps:** {config.TTT_STEPS}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'ttt_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>TTT score: {etf['ttt_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized TTT Score"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized TTT Score", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🧠 Top ETFs by Test-Time Training × Momentum (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Test-Time Training (TTT)** networks continue to update the last layers during inference.
        - A **self-supervised loss** (reconstruction) adapts the model to distribution shift in real‑time.
        - The model learns from sequences of ETF returns and macro variables.
        - **Momentum factor:** 1 + last_return (clipped to 0.5–2.0).
        - **Final score = TTT prediction × momentum** – adapts to changing market conditions.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the TTT scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Test-Time Training | Self-supervised adaptation during inference × momentum")
